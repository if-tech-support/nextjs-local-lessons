...
- 子要素として次をこの順で配置する
  1. 見出し：`<h1 className="text-[24px] font-bold transition-all duration-300 hover:opacity-70">`
  - 中に`<Link href="/">Web World Creators</Link>`を入れる
  2. ナビゲーション：`<nav className="flex items-center gap-3">`
  - 中に`<Link href="/company" className="transition-opacity duration-200 hover:opacity-70">会社概要</Link>`
  - 続けて`<Link href="/service" className="transition-opacity duration-200 hover:opacity-70">サービス紹介</Link>`
- 追加の余白やラッパーは作らない（指定の構造・クラスのみ）
