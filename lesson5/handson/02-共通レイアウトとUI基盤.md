...
単一の`layout.tsx`で`html`/`body`/主要ラッパー/`main`/`header`/`footer`の最小構造を作り画面全体のレイアウトを整える。
...
- 追加コンポーネント: `Header`関数コンポーネント（ローカル）
