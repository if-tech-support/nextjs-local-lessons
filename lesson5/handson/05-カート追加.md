...
カートへ商品を1点追加して、既存なら数量+1するServer Actionを作る。

- 対象ファイル:`src/lib/actions.ts`（新規作成）
- サーバーサイド実行の宣言: 先頭に`'use server'`
- ユーザー識別関数: `tempUserId()`で`'demo-user'`を返却
  - カートに紐づくユーザーはすべてダミーのユーザーに紐付けるためIDを`'demo-user'`とする
- 関数: `addToCartAction(FormData)`をエクスポート
  - 入力: `FormData`から`id`を文字列取得
  - 不正入力: `id`が文字列でない時は何もせずリターン
  - 既存判定: `cart_items`を`temp_user_id`=`demo-user`かつ`product_id`=`id`で1件取得
    - 既存ありの場合: `quantity`を+1に`update`
    - 既存なしの場合: `{ temp_user_id:'demo-user', product_id:id, quantity:1 }`を`insert`
  - エラー: データベース操作（取得/更新/挿入）の`error`があればthrow
  - 共通処理: `finally`で`/products` `/orders` `/cart`を`revalidatePath`で再検証
  - 戻り値: なし
  - 例外時: `console.error(e)`
...
商品一覧の各カードにカート追加フォームを付与し数量を加算する。
