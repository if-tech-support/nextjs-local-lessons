...
カート内の全商品を1回の操作で注文しカートを空にして注文ページへ遷移させる。
...
- 関数: `placeOrderAction`
  - 固定ユーザーID: `tempUserId()` で `demo-user` を取得する。
  - 動作: カートテーブルから `id` `product_id` `quantity` `products(price)` を `temp_user_id='demo-user'` 条件で全件取得して、`data`を`cartItems`の別名にする。
  - 動作: 取得結果が0件なら `/orders?empty=1` へ `redirect` する。
  - 動作: 注文テーブルに `{ temp_user_id: 'demo-user' }` を挿入し `id` を取得して、`data`を`order`の別名にする。
  - 動作: 取得結果の配列を`map`メソッドで操作して、1件ずつ`{ order_id, product_id, quantity, price_at_order }`を作り、`order_items`へ一括挿入する（`price_at_order`は価格取得不可時0）。取得結果の配列の型は`type CartRow = { id: string; product_id: string; quantity: number; products: { price: number } | null`と定義する。
  - 成功時: `revalidatePath('/orders')` → `revalidatePath('/products')` → `revalidatePath('/cart')` の順で呼ぶ。
  - 成功後: 同一ユーザー条件で `cart_items`テーブルから削除する（失敗時は削除しない）。`/orders?placed=<注文id>` へ `redirect` する。
  - 例外時:  `console.error(e)`
  - 共通処理: `/orders` `/products` `/cart` を再検証する（重複呼び出し可）。
