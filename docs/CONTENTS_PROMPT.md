...
12) chapters
   ...
   - 拡張フォーマット: 1行完結形式 `n. \`filename\`（短い表示名）: 対象ファイル=pathA pathB ... / 後続章: 未実装要素列挙` を許容。以下条件をすべて満たす場合にこの拡張を自動採用する:
      1. 章ファイル冒頭段落内に対象コードファイルを明示（インラインコード）
      2. 同一ファイルを複数章で段階的に扱う意図がdesignソースまたは既出章で確認できる
      3. "後続章" 部分に列挙する要素はいずれも現行章では実装・解説しないことが章本文で明確
   - 拡張フォーマット要素定義:
      - `（短い表示名）`: 章タイトルを10〜14文字程度に要約（冗長語削除）。存在しない場合は省略可。
      - `対象ファイル=`: 当該章で直接編集または新規作成/追記する主要ファイル（最大5）。サブディレクトリや補助的importのみのファイルは含めない。複数は半角スペース区切り・順序は `src/app/**` → `src/lib/**` → `supabase/**` → 設定ファイル。
      - `後続章:`: 次以降の章で扱うことを明示する未実装機能・UI要素・Action名を簡潔列挙（読点/スラッシュ使わず全角中点・読点かスペース区切り）。ゼロ件の場合はこのセクション全体を省略。
   - Server Actionはファイル側に実在する命名のみ（例:`addToCartAction`）。
   - 未実装列挙内にすでにコード存在している要素を含めない（Strict Parityで除外対象）。
   - 拡張フォーマット採用時は従来の次行インデント形式と混在させない（全章統一）。
...
【Strict Chapter Parity（章整合性追加規則）】
   ...
   - ただし拡張フォーマット使用時の `後続章:` セグメントは例外的に未来未実装要素の列挙を許容する。この列挙:
      1. 主概要文（コロンより前）には含めない
      2. すでにコードに存在する機能名/Action名を含めない（存在する場合は削除）
      3. 名詞または簡潔な機能ラベルに限定（文章化しない）
      4. Strict Parity差分評価から除外しLATER_CHAPTER_ELEMENTとして扱う
      5. 同一語が複数章の `後続章:` に残存する場合は最初に実装した章以降で自動削除
