...
- 許容差異: フォーマッタによる空白/改行のみ。
 ...
 - 自動検査フロー（執筆内部手順）:
	 ...
	 5. 差分が後続章対象によるものか判定。後続章対象なら無視、それ以外は教材側を再生成して整合。
	 6. 占位が必要な箇所は JSX コメント（`{/* 後続課題で〜 */}`）で明示。
...
生成時は以下カテゴリ分類を想定し差分ゼロへ調整する（後続章導入要素は除外可）。
...
［生成ワークフロー追補］
1. 課題/章本文執筆 -> 2. 完全再現性チェック通過 -> 3. Strict Parity差分確認 -> 4. 差分が後続章要素のみなら保持し注記不要 / それ以外は教材コード側修正 -> 5. 最終提出。
