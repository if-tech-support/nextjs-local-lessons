...
【追加自動読込処理（contents/designクロスチェック）】
- フェーズ2開始時に、対象章ファイルと同一ディレクトリ（`.../handson/`）内の `contents.md` と `design.md` を自動で読み取る（存在する場合）。
- これらは章全体の目次整合・デザイン仕様（色/余白/コンポーネント命名）の参照ソースとし、章Markdownとの不整合を検出する。
- 不整合は既存基準番号を増やさず、以下へマッピングして指摘する:
   - 章構成・見出し順序/タイトル差異 → 基準1（構成）
    - 学習対象スキル一覧差異（`contents.md`内の該当章行と比較）→基準1または15（完全再現性）
    - セクション/コンポーネント命名・表示テキスト差異（`design.md`）→基準13（Strict Parity追加詳細）+必要に応じてSPEC_MISMATCH行
   - デザイン数値（色コード/px/余白/並び順）差異 → 基準15（完全再現性）
- `contents.md` / `design.md` が存在しない場合は「参考ファイル欠落」として情報メモに留め、欠落自体でFAILにしない（該当差分検証をスキップ）。
- 差分指摘行の形式は従来の「修正項目」ルールを踏襲し、修正対象は必ず章Markdownのみ（`contents.md` / `design.md` 側を変更する指示は出さない）。
- `contents.md` / `design.md` を更新すべきと判断できる事象でも、本レビューでは教材章Markdownへの追従修正を原則とする（アプリ非改変と同列方針）。
...
- 例外はありません（将来導入予定要素が先行実装されていてもアプリ非改変を維持。学習者向け本文に「後続章」「次章で〜」等の将来参照は記載しない）。
...
1. 構成
   ...
   - 目的節: 箇条書き禁止。平文1〜3文で章全体の狙いを簡潔に要約（末尾句点必須）
...
2. 仕様記述（タイプA課題内のみ判定。Bは参考、CはN/A）
   ...
   - 命名明示: 使用・新規作成する関数/コンポーネント名/変数・定数/配列名/状態フック名/イベントハンドラー名/Route Handler名（GET/POST等）/フォームフィールド名/型・インターフェイス名を列挙し、学習者が仕様のみで解答例コードの命名を完全再現できる（曖昧表現:「任意」「自由な名前」「好きな名前」禁止）。列挙は仕様本文の箇条書き内、もしくは本文直後に独立ブロック「【命名】」を置きハイフン箇条書きで明示（親カテゴリ:→具体名形式可）。
